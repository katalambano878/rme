#!/usr/bin/env python3
"""Restore badawiasimports dump into badawias_staging on fleet-postgres."""
import json
import os
import subprocess
import sys
...
DUMP = Path(os.environ.get("BI_DUMP_DIR", "/data/coolify/badawias-staging/dumps"))
DB = os.environ.get("BI_DB_NAME", "badawias_staging")
ROLE = os.environ.get("BI_DB_ROLE", DB)
SUPABASE_HOST = os.environ.get(
    "BI_SUPABASE_HOST", "https://cddyzkiosfukthugsyhu.supabase.co"
)
...
def main():
    ...
    UNION ALL SELECT 'contact_submissions', count(*)::int FROM contact_submissions
    ORDER BY 1;
    """
    )
    print(out)
