...
<p style="margin:0 0 8px;color:#6b7280;font-size:13px;">Need help? <a href="${BRAND.url}/contact" style="color:${BRAND.color};text-decoration:none;">Contact us</a></p>
<p style="margin:0 0 12px;color:#6b7280;font-size:13px;"><a href="${BRAND.url}" style="color:${BRAND.color};text-decoration:none;">Visit our store</a> &nbsp;·&nbsp; <a href="${BRAND.url}/track-order" style="color:${BRAND.color};text-decoration:none;">Track order</a></p>
...
export async function sendSMS({ to, message }: { to: string; message: string }) {
    ...
    if (digits.length < 10 || digits.length > 15) {
        console.warn('[SMS] Invalid phone format, skipping send');
        return null;
    }
...
export async function sendOrderConfirmation(order: any) {
    const { id, email, phone: orderPhone, shipping_address, total, grand_total, created_at, order_number, metadata } = order;

    const orderTotal = total ?? grand_total;
    ...
    const trackingUrl = `${baseUrl}/track-order?order=${order_number || id}`;
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:12px;overflow:hidden;margin:20px 0;">
  ...
  ${emailInfoRow('Total', `GH&#8373;${Number(orderTotal).toFixed(2)}`)}
</table>
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:12px;overflow:hidden;margin:16px 0;">
  ...
  ${emailInfoRow('Email', email || 'N/A')}
  ${emailInfoRow('Phone', phone || 'N/A')}
  ${emailInfoRow('Total', `GH&#8373;${Number(orderTotal).toFixed(2)}`)}
  ${trackingNumber ? emailInfoRow('Tracking', trackingNumber) : ''}
</table>
...
`, `New order #${order_number} from ${name}`)
    ...
    // 4. SMS to Admin numbers
    if (ADMIN_SMS_NUMBERS.length > 0) {
        const adminSmsBody = `New order #${order_number || id} from ${name}. Total: GH&#8373;${Number(orderTotal).toFixed(2)}. View: ${baseUrl}/admin/orders/${id}`;
        for (const adminPhone of ADMIN_SMS_NUMBERS) {
            await sendSMS({ to: adminPhone, message: adminSmsBody });
        }
    }
}
...
export async function sendPaymentLink(order: any) {
    const { id, email, phone: orderPhone, shipping_address, total, grand_total, order_number, metadata } = order;
    const orderTotal = total ?? grand_total;
    const baseUrl = BRAND.url;
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background-color:#f9fafb;border-radius:12px;overflow:hidden;margin:20px 0;">
  ...
  ${emailInfoRow('Amount Due', `<span style="color:${BRAND.color};font-size:18px;font-weight:700;">GH&#8373;${Number(orderTotal).toFixed(2)}</span>`)}
</table>

<p style="color:#374151;font-size:14px;line-height:1.6;margin:16px 0;">Click the button below to securely complete your payment.</p>

${emailButton(`Pay Now — GH&#8373;${Number(orderTotal).toFixed(2)}`, paymentUrl, '#d97706')}
...
`, `Complete payment for order #${order_number}`)
    ...
    if (phone) {
        await sendSMS({ to: phone, message: `Hi ${name}, complete your order #${order_number} (GH₵${Number(orderTotal).toFixed(2)}) here: ${paymentUrl}` });
    }
}
