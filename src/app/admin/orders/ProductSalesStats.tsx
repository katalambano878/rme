            ...
            if (data) {
                ...
                data.forEach((item: any) => {
                    ...
                    if (!map.has(pid)) {
                        map.set(pid, {
                            productId: pid,
                            productName: item.name_snapshot || 'Unknown',
                            ordersCount: 0,
                            itemsSold: 0,
                            totalRevenue: 0,
                            _orderIds: new Set(),
                    ...
                    const lineTotal = Number(item.line_total) || (Number(item.unit_price) * (item.quantity || 0));
                    entry.itemsSold += (item.quantity || 0);
                    entry.totalRevenue += lineTotal;

                    const opts = item.options_snapshot as Record<string, string> | null;
                    const variantParts = opts
                      ? [opts.size, opts.color].filter(Boolean)
                      : [];
                    const variantName = variantParts.length > 0
                      ? variantParts.join(' · ')
                      : (item.sku_snapshot || 'Default');
