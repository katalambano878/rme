...
export default function OrderDetailClient({ orderId }: OrderDetailClientProps) {
  ...
  const fetchOrderDetails = async () => {
    try {
      ...
      const normalizedOrder = {
        ...
        payment_status: paidPayment ? 'paid' : 'pending',
        payment_method: paidPayment?.provider || data.payments?.[0]?.provider || 'N/A',
        payment_transaction_id: paidPayment?.provider_ref || '',
        shipping_method: 'Standard',
        metadata: {},
        order_items: (data.order_items || []).map((item: any) => {
          const opts = item.options_snapshot as Record<string, string> | null
          const variantParts = [opts?.size, opts?.color].filter(Boolean)
          return {
            ...item,
            product_name: item.name_snapshot,
            variant_name: variantParts.length ? variantParts.join(' · ') : '',
            sku: item.sku_snapshot || '',
            total_price: Number(item.line_total) || (Number(item.unit_price) * item.quantity),
          }
        }),
      };
  ...
  // Normalize shipping address so the UI works regardless of which checkout
  // flow created the order (storefront / chat / POS / legacy).
  const shippingAddress = normalizeShippingAddress(order.shipping_address);
  const customerName = shippingAddress.fullName || order.email?.split('@')[0] || 'Customer';
  ...
  const fraudAnalysis: FraudAnalysis = (() => {
    ...
    if (total > 1500) reasons.push(`Very high order value (GH₵${total.toFixed(2)})`);
    else if (total > 600) reasons.push(`High order value (GH₵${total.toFixed(2)})`);

    if (isGuest && total > 300) reasons.push('Guest checkout with high-value order');
    if (totalQty > 10) reasons.push(`Large quantity order (${totalQty} items)`);
    if ((order._recentOrderCount ?? 0) > 2) reasons.push(`${order._recentOrderCount} orders from this email in last 48 h`);

    if (!addressComplete) reasons.push('Incomplete shipping address');

    const riskLevel: RiskLevel =
      reasons.filter(r => r.includes('Very high') || r.includes('orders from')).length > 0 ? 'high'
      ...
      <div className="max-w-7xl mx-auto p-6 no-print">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
            {/* Fraud detection temporarily disabled. Flip FRAUD_DETECTION_ENABLED to true to re-enable. */}
            {FRAUD_DETECTION_ENABLED && fraudAnalysis.riskLevel !== 'low' && (
              <FraudDetectionAlert
                  ...
                  <div key={item.id} className="flex items-start space-x-4 p-4 bg-gray-50 rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p
                        ...
                        title={item.sku ? `Stored SKU: ${item.sku}` : undefined}
                      >
                        SKU: {buildDisplaySku(item.product_name || '', item.sku || '')}
                      </p>
                    </div>
                    <div className="text-right">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
              ...
              {addressComplete ? (
                ...
                <div className="rounded-lg border border-amber-300 bg-amber-50 p-3 text-sm text-amber-900">
                  No shipping address on file for this order. The customer may
                  have selected store pickup, or completed checkout before this
                  field was captured.
                </div>
              )}
            </div>
