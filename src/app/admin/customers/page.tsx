  ...
  const fetchCustomersFromProfiles = async () => {
    try {
      ...
      const registeredCustomers = (profiles || []).map((profile: any) => {
        ...
        const totalSpent = userOrders.reduce((sum, o) => sum + Number(o.grand_total || 0), 0);
        ...
        let status = 'New';
        if (totalSpent > VIP_THRESHOLD) status = 'VIP';
      ...
      const guestOrders = orders?.filter(o => !o.user_id && o.guest_email) || [];
      const guestMap = new Map<string, any>();

      guestOrders.forEach(order => {
        const existing = guestMap.get(order.guest_email);
        ...
        if (!existing) {
          guestMap.set(order.guest_email, {
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={() => {
            const rows = [
              ['Name', 'Email', 'Phone', 'Orders', 'Total Spent (GHS)', 'Status', 'Joined', 'Last Order'],
              ...customers.map(c => [
                c.name, c.email, c.phone,
                c.orders, c.totalSpent.toFixed(2),
                c.status, c.joined, c.lastOrder,
              ]),
            ];
