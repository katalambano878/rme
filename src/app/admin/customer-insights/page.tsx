...
const AT_RISK_DAYS       = 90    // Days since last order to flag as at-risk
const NEW_CUSTOMER_DAYS  = 30    // Days since joining to count as "new"
  ...
  const fetchCustomerData = async () => {
    try {
      ...
      const aggregated = (profiles || []).map((profile: any) => {
        ...
        if (totalSpent > VIP_THRESHOLD) segment = 'vip';
        else if (orderCount > 1) segment = 'returning';
        else if (daysSinceLastOrder > AT_RISK_DAYS && orderCount > 0) segment = 'at-risk';
        else if (daysSinceJoin < NEW_CUSTOMER_DAYS) segment = 'new';
