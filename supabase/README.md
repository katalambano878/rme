...
3. Apply later migrations in timestamp order (`migrations/*.sql`), including `20260803120000_rebrand_trust_ecom_defaults.sql`.
4. Optionally run `seed.sql` for starter categories and Trust Ecom settings.
...
insert into public.store_admins (email) values ('hello@trustecom.com')
  on conflict (email) do nothing;
```
